...
\item Cities are defined by their GDP and their profile regarding the proportion of firms in the different sectors. Links between cities are created in an iterative way, taking into account:
...
\begin{enumerate}
    \item Generate $N=700$ cities with size following a power law $E_i \eq E_0 \cdot i^{-\alpha}$ with $E_0 \eq 10^{11}$ and $\alpha=1.1$ (computed on Europe for GDP with cities larger than 50.000 inhabitants)
