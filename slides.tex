...
% Wij c'est quoi? il me semble que tu as dit que c'est le lien du passé et Sik c'est la structure economique defini par le cosine similarity
...
\begin{enumerate}
    \item Generate $N \textrm{ = } 700$ cities with size following a power law $E_i \textrm{ = } E_0 \cdot i^{-\alpha}$ with $E_0 \textrm{ = } 10^{11}$ and $\alpha=1.1$ (computed on Europe for GDP with cities larger than 50.000 inhabitants)
