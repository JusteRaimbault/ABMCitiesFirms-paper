...
%%%%%%
% calibration: full model - different Pareto front shape: ? - for later

% -> several things to investigate regarding calib: comparison with stat model - perf of random sampling for the sim model, with countries: constraint using stat model result?
% with countries - more complic ated front: ?

% - test other constraints - perf % stat models?
% - rescaling <=> w0 : still holds with evolving cities? not sure - to be investigated - here ok since only one term involving weights, and rescaling cancels out.
%  -> specific paper on calib of simu model - comparison ith stat model; more work on empirical AIC?
...
%  in progress: test macro calib ; rerun explo metropolisation? -> ok
%  -> check results, compare to grid results
