...
It is worth mentioning that our model is not aimed to be predictive, but rather designed as an explanatory model in order to disentangle the influence of diverse processes in the growth of the urban network and to test diverse hypothesis \cite{conte2012manifesto}. It also tends to be a decision-making model as we explore potential policy applications. Our focus on explanation rather than prediction can be understood in a context of a broad diversity of possible model functions \cite{varenne2018models}.
...
% TODO recall not a predictive model
...
Additional zero-inflated and hurdle statistical models; details on the simulation model: summary statistics for the socio-economic distance matrix, procedure for the synthetic model setup and detailed Global Sensitivity Analysis.
